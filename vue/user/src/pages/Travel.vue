<template>
        ...
        <view class="block">
          <div class="article">
            ...
            <div v-if="$route.query.mode=='骑行'">
              &nbsp;&nbsp;骑行，是与风共舞的艺术，是穿梭于城市街巷的浪漫诗篇。每一次脚踏踏实的旋转，都是对自由的渴望与追求。骑行吧，让我们以车轮为笔，以道路为纸，书写绿色篇章。
            </div>
          </div>
  ...
  <van-button type="primary" size="large" v-if="showEnd" @click="endTravel">结束出行</van-button>
</template>
...
const endTravel = async () => {
  form.endTime = new Date().toLocaleString()
  const data = await addTravel(form)
  if(data==1){
    router.push({ name: 'travelMode' })
  }
}
</script>
